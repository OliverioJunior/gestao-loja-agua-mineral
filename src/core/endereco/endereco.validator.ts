...
import { EnderecoValidation } from "./endereco.errors";
...
export class EnderecoValidator {
  ...
  static validateId(id: string) {
    if (!id) {
      throw new EnderecoValidation("id", id, "id_required");
    }
    return { id, validate: true };
  }
  static validateLogradouro(logradouro: string) {
    if (!logradouro?.trim()) {
      throw new EnderecoValidation(
        "logradouro",
        logradouro,
        "logradouro_required"
      );
  ...
  private static validateCEP(zipCode: string) {
    if (!zipCode?.trim()) {
      throw new EnderecoValidation("cep", zipCode, "cep_required");
    }
    if (!this.ZIP_CODE_REGEX.test(zipCode.trim())) {
      throw new EnderecoValidation("cep", zipCode, "cep_invalid");
    }
  }
  ...
  private static validateNumero(numero: string) {
    if (!numero?.trim()) {
      throw new EnderecoValidation("numero", numero, "numero_required");
    }
  }
  ...
  private static validateBairro(bairro: string) {
    if (!bairro?.trim()) {
      throw new EnderecoValidation("bairro", bairro, "bairro_required");
    }
  }
  private static validateCidade(cidade: string) {
    if (!cidade?.trim()) {
      throw new EnderecoValidation("cidade", cidade, "cidade_required");
    }
  }
  private static validateEstado(estado: string) {
    if (!estado?.trim()) {
      throw new EnderecoValidation("estado", estado, "estado_required");
    }
  }
  private static validateTipo(tipo: string) {
    if (!tipo?.trim()) {
      throw new EnderecoValidation("tipo", tipo, "tipo_required");
    }
  }
