...
export class Pedido {
  ...
  static create(data: CreatePedidoInput) {
    return PedidoValidation.validateCreateInput(data);
  }
  ...
  updateStatus(novoStatus: StatusPedido): void {
    PedidoValidation.validateStatusTransition(this.data.status, novoStatus);
    this.data.status = novoStatus;
  }

  updateTotal(novoTotal: number): void {
    PedidoValidation.validateUpdateInput({ total: novoTotal });
