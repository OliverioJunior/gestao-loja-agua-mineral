...
import { ProdutoValidationError } from "./produto.errors";

export class ProdutoValidation {
  ...
  static validateId(id: string) {
    if (!id) {
      throw new ProdutoValidationError("id", id, "id_required");
    }
  ...
  private static validateNome(nome: string) {
    if (!nome?.trim()) {
      throw new ProdutoValidationError("nome", nome, "nome_required");
    }
    if (nome.trim().length < this.MIN_NAME_LENGTH) {
      throw new ProdutoValidationError("nome", nome, "nome_min_length");
    }
  }
  ...
  private static validatePrecoPositivo(preco: number, campo: string) {
    if (preco <= 0) {
      throw new ProdutoValidationError(campo, preco, "preco_positive");
    }
  }

  private static validatePrecoMaximo(preco: number, campo: string) {
    if (preco / 100 > this.MAX_PRICE) {
      throw new ProdutoValidationError(campo, preco, "preco_max_value");
    }
  }
  ...
  private static validateAtLeastOneField(data: UpdateProdutoInput) {
    ...
    if (!hasAtLeastOneField) {
      throw new ProdutoValidationError(
        "produto",
        data,
