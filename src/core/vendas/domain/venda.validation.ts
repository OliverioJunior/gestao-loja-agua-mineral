...
import { VendaValidationError } from "./venda.errors";

export class VendaValidation {
  ...
  static validateId(id: string) {
    if (!id?.trim()) {
      throw new VendaValidationError("id", id, "id_required");
    }
    return { id, validate: true };
  }
  private static validateTotal(total: number) {
    if (total === undefined || total === null) {
      throw new VendaValidationError("total", total, "total_required");
    }

    if (typeof total !== "number" || total < this.MIN_TOTAL) {
      throw new VendaValidationError("total", total, "total_positive");
    }
  ...
  private static validateFormaPagamento(formaPagamento: FormaPagamentoVenda) {
    if (!this.VALID_FORMA_PAGAMENTO.includes(formaPagamento)) {
      throw new VendaValidationError(
        "formaPagamento",
        formaPagamento,
  ...
  private static validateAtLeastOneField(data: UpdateVendaInput) {
    ...
    if (!hasAtLeastOneField) {
      throw new VendaValidationError("venda", data, "at_least_one_field");
    }
  }
